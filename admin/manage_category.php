...
if(isset($_POST['submit'])){
	...
	if($id==''){
		...
		$sql="select * from category where category_name='$category' and id!='$id'";
	}
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){
		$msg="Category already added";
	}else{
		if($id==''){
			mysqli_query($con,"INSERT INTO `category`(`category_name`, `order_number`, `stats`) values('$category','$order_number','1')");
		}else{
			mysqli_query($con,"update category set category_name='$category', order_number='$order_number' where id='$id'");
		}
